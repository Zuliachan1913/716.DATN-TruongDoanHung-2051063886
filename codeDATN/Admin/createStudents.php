...
if (isset($_POST['save'])) {
    ...
    // Kiểm tra trùng admissionNumber
    ...
    if ($check->num_rows > 0) {
        $statusMsg = "<div class='alert alert-danger'>Số ID này đã tồn tại!</div>";
    } else {
        $stmt = $conn->prepare("INSERT INTO tblstudents 
            (firstName, lastName, otherName, admissionNumber, password, classId, classArmId, dateCreated) 
            VALUES (?, ?, ?, ?, '12345', ?, ?, ?)");
        $stmt->bind_param("sssssss", $firstName, $lastName, $otherName, $admissionNumber, $classId, $classArmId, $dateCreated);

        if ($stmt->execute()) {
            $statusMsg = "<div class='alert alert-success'>Thêm thành công!</div>";
        } else {
            $statusMsg = "<div class='alert alert-danger'>Lỗi: " . $stmt->error . "</div>";
    }
  }
}
...
if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit") {
 ...
 if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE tblstudents SET 
        firstName=?, lastName=?, otherName=?, admissionNumber=?, 
        password='12345', classId=?, classArmId=? 
        WHERE Id=?");
    $stmt->bind_param("ssssssi", $firstName, $lastName, $otherName, $admissionNumber, $classId, $classArmId, $Id);

    if ($stmt->execute()) {
        echo "<script>window.location='createStudents.php'</script>";
    } else {
        $statusMsg = "<div class='alert alert-danger'>Lỗi cập nhật!</div>";
    }
  }
}
